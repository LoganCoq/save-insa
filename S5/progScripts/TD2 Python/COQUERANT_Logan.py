...
def daysPerMonths(month, year):
    """
        ...
        CU : The year needs to be higher than 1582 because it's the time where the Gregorian  	
	calendar took effect
        Also the month needs to be a valid month
    """
    assert month < 13
    m30 = [4,6,9,11]
    if month == 2:
...
print(daysPerMonths(7,2017))

# 1.4.3

def is_valid_date(y,m,d):
    """
	This function indicates if a date is valid or not. 
    """
    assert m < 13 , "Date non valide"
    assert d < daysPerMonths(m, y) + 1, "Date non valide"

    return True

#print(is_valid_date(2016, 2, 29))
#print(is_valid_date(2017, 2, 29))

def is_valid_dateV2(y, m, d):
    """
	This function does the same thing as 'is_valid_date', which is to tell whether the date is valid or not.
	Except this function uses conditional instead of asserts.
    """
    if m > 12 or d > daysPerMonths(m, y):
        return False
    else:
        return True

print(is_valid_dateV2(2016,2,29))
print(is_valid_dateV2(2015,4,26))
