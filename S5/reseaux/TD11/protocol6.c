...
/* Construct and send a data, ack, or nak frame. */
  ...
  if (fk == data) start_timer(frame_nr % NB_BUFS);
...
void protocol6(void)
{
  ...
  packet out_buf[NB_BUFS];        /* buffers for the outbound stream */
  packet in_buf[NB_BUFS];        /* buffers for the inbound stream */
  boolean arrived[NB_BUFS];        /* inbound bit map */
  ...
  too_far = NB_BUFS;        /* receiver's upper window + 1 */
  nbuffered = 0;        /* initially no packets are buffered */

  for (i = 0; i < NB_BUFS; i++) arrived[i] = false;
  ...
  while (true) {
     ...
     switch(event) {
        case network_layer_ready:        /* accept, save, and transmit a new frame */
                ...
                from_network_layer(&out_buf[next_frame_to_send % NB_BUFS]); /* fetch new packet */
        ...
        case frame_arrival:        /* a data or control frame has arrived */
                ...
                if (r.kind == data) {
                        ...
                        if (between(frame_expected, r.seq, too_far) && (arrived[r.seq%NB_BUFS] == false)) {
                                /* Frames may be accepted in any order. */
                                arrived[r.seq % NB_BUFS] = true;        /* mark buffer as full */
                                in_buf[r.seq % NB_BUFS] = r.info;        /* insert data into buffer */
                                while (arrived[frame_expected % NB_BUFS]) {
                                        /* Pass frames and advance window. */
                                        to_network_layer(&in_buf[frame_expected % NB_BUFS]);
                                        no_nak = true;
                                        arrived[frame_expected % NB_BUFS] = false;
                ...
                while (between(ack_expected, r.ack, next_frame_to_send)) {
                        ...
                        stop_timer(ack_expected % NB_BUFS);        /* frame arrived intact */
     ...
     if (nbuffered < NB_BUFS) enable_network_layer(); else disable_network_layer();
  }
}
