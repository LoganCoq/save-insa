...
int main(int argc, const char *argv[])
{
	...
	int nSD;
	if((nSD = socket(AF_INET, SOCK_DGRAM, 0)) < 0) // TO BE COMPLETED FOR HAVING A DATAGRAM TRANSMISSION
	{
		perror("socket()");
		exit(1);
	}
	...
	if(bind(nSD, (struct sockaddr *)&server, sizeof(server)) < 0) // TO BE COMPLETED
	{
		perror("bind()");
		exit(3);
	}
	...
	if(setsockopt(nSD, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) // TO BE COMPLETED
	{
		perror("setsockopt()");
		exit(4);
	}
...
/*
2.3)
La valeur de cette adresse multicast est 255.0.0.37

2.4)

2.5)
Pour tester le server et le client, on lance le multicastReceiver pour ensuite lancer le multicastSender avec un argument qui correspondra au message que l'on souhaite envoyer.
*/
