...
{
msgctl (id_file, IPC_RMID, NULL); // remove id
close(fd1);
close(fd2);
}
...
if (( id_file = msgget (clef, IPC_CREAT | IPC_EXCL | 0666)) == -1) { //creation file, file non existante
	// 0xxx -> indique que la valeur qui suit est octale
	close (fd1);
	close (fd2);
