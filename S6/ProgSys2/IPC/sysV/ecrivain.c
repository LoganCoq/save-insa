...
{
	int i;
	semop(semid, demande_ecriture,2);	/* 2 opérations */
...
void init_ecrivain()
{
	...
	if ((semid = semget(clef_sem,2,IPC_CREAT|IPC_EXCL|0666))==-1) /* crée 2 sem*/
...
int     i,
        j;
 if ((addr = shmat(shmid,NULL,0))==(char *)-1) /* attache la memoire partagée */
